        ...
        <div className="mt-10 grid grid-cols-1 gap-10 md:grid-cols-12 md:gap-16">
          ...
          <div className="md:col-span-5 md:pt-10">
            ...
            <div className="mt-10 flex flex-col gap-4">
              {isArtwork ? <SnipcartButton product={product} label="Buy Original" price={2500} stock={1} /> : null}
              {isVariable ? <SnipcartButton product={product} label={isArtwork ? "Add Fine Art Print to Cart" : "Add to Cart"} price={itemPrice} customFields={isArtwork ? { name1: "Material", options1: product.materials?.join("|") ?? "", name2: "Size", options2: product.sizes?.join("|") ?? "" } : { name1: "Size", options1: product.sizes?.join("|") ?? "", name2: "Color", options2: product.colors?.join("|") ?? "" }} /> : null}
              {isArtwork ? <a href={`mailto:stathestudio@gmail.com?subject=Recommission ${product.title}`} className="border-b border-ink px-1 py-3 text-center font-sans text-[11px] uppercase tracking-[0.2em] transition-colors hover:border-brand-blue hover:text-brand-blue">Recommission this Artwork →</a> : null}
              {!isVariable ? <SnipcartButton product={product} label="Add to Cart" price={product.price} /> : null}
            </div>
...
function SnipcartButton({ product, label, price, stock, customFields }: { product: ShopProduct; label: string; price: number; stock?: number; customFields?: { name1: string; options1: string; name2: string; options2: string } }) {
  return <Button className="snipcart-add-item w-full rounded-none bg-ink py-6 font-sans text-[11px] uppercase tracking-[0.2em] text-white hover:bg-brand-blue" data-item-id={product.id} data-item-name={product.title} data-item-price={price} data-item-url={`/shop/product/${product.id}`} data-item-description={product.description} data-item-image={product.image} data-item-stock={stock} data-item-custom1-name={customFields?.name1} data-item-custom1-options={customFields?.options1} data-item-custom2-name={customFields?.name2} data-item-custom2-options={customFields?.options2}>{label} ↗</Button>;
}
